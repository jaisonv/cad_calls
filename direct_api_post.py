...
def _cleanup_history(results_dir, site_name, agency_id):
    """Keep only a small rolling history of result/debug files."""

    def prune(prefix, suffix, keep):
        files = []
        ...
        for old_path in files[keep:]:
            try:
                ...
                logger.warning(f"Failed to remove old file {old_path}: {exc}")

    base_prefix = f"{site_name}_cadcalls_{agency_id}_"
    debug_prefix = f"{site_name}_debug_info_"

    prune(base_prefix, ".json", MAX_JSON_HISTORY)
    prune(base_prefix, ".txt", MAX_TEXT_HISTORY)
    prune(base_prefix, ".html", MAX_HTML_HISTORY)
    prune(debug_prefix, ".txt", MAX_DEBUG_HISTORY)


def make_cadcalls_request(
    agency_id=AGENCY_ID,
    include_open=DEFAULT_PARAMS["include_open"],
    include_closed=DEFAULT_PARAMS["include_closed"],
    take=DEFAULT_PARAMS["take"],
    skip=DEFAULT_PARAMS["skip"],
    search_text=DEFAULT_PARAMS["search_text"],
):
    """
    ...
    try:
        cadcalls_response = session.get(
            cad_url, timeout=API_SETTINGS.get("timeout", 30)
        )
        ...
        try:
            ...
            cadcalls_response = session.get(
                alternative_cad_url, timeout=API_SETTINGS.get("timeout", 30)
            )
            cadcalls_response.raise_for_status()
            logger.info(
                f"Successfully accessed alternative CADCalls page at {alternative_cad_url}"
            )
        except requests.exceptions.RequestException as e:
    ...
    if "XSRF-TOKEN" in session.cookies:
        ...
        logger.warning(
            "No CSRF token found in cookies. Some sites may require this for API access."
        )
    ...
    try:
        if request_method == "POST":
            logger.info(
                f"Using POST method with payload: {json.dumps(payload, indent=2)}"
            )
            response = session.post(
                api_url, json=payload, timeout=API_SETTINGS.get("timeout", 30)
            )
        else:  # GET
            ...
            response = session.get(
                api_url, params=params, timeout=API_SETTINGS.get("timeout", 30)
            )
        ...
        _cleanup_history("cadcalls_results", site_name, agency_id)
    ...
    if "CADCalls" in data and isinstance(data["CADCalls"], list):
        ...
        for i, call in enumerate(calls, 1):
            ...
            if call.get("HasLocation"):
                print(
                    f"  Location: ({call.get('Latitude', 'Unknown')}, {call.get('Longitude', 'Unknown')})"
                )
            print()
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fetch CAD Calls data from Tyler Police-to-Citizen portal"
    )
    parser.add_argument(
        "--agency-id",
        type=int,
        default=AGENCY_ID,
        help=f"Agency ID (default: {AGENCY_ID})",
    )
    parser.add_argument(
        "--open",
        action="store_true",
        default=DEFAULT_PARAMS["include_open"],
        help=f"Include open calls (default: {DEFAULT_PARAMS['include_open']})",
    )
    parser.add_argument(
        "--closed",
        action="store_true",
        default=DEFAULT_PARAMS["include_closed"],
        help=f"Include closed calls (default: {DEFAULT_PARAMS['include_closed']})",
    )
    parser.add_argument(
        "--take",
        type=int,
        default=DEFAULT_PARAMS["take"],
        help=f"Number of records to retrieve (default: {DEFAULT_PARAMS['take']})",
    )
    parser.add_argument(
        "--skip",
        type=int,
        default=DEFAULT_PARAMS["skip"],
        help=f"Number of records to skip (default: {DEFAULT_PARAMS['skip']})",
    )
    parser.add_argument(
        "--search",
        type=str,
        default=DEFAULT_PARAMS["search_text"],
        help="Search text to filter calls",
    )
    parser.add_argument(
